...
if (supabaseUrl && supabaseServiceKey) {
  ...
  try {
    ...
    console.error("❌ [Socket Server] Invalid Supabase URL format:", error.message);
    console.error("   NEXT_PUBLIC_SUPABASE_URL:", supabaseUrl ? `"${supabaseUrl}"` : "not set");
    console.warn("⚠️ [Socket Server] Device authentication will be disabled due to invalid configuration.");
  }
} else {
  if (!supabaseUrl) {
    console.warn("⚠️ [Socket Server] NEXT_PUBLIC_SUPABASE_URL not found. Device authentication will be disabled.");
  }
  if (!supabaseServiceKey) {
    console.warn("⚠️ [Socket Server] SUPABASE_SERVICE_ROLE_KEY not found. Device authentication will be disabled.");
  }
}
